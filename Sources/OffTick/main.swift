...
struct WorkCalendar {
    ...
    private static func isWorkday(_ date: Date, regionCode: String?) -> Bool {
        ...
        if publicHolidays2026ByRegion[regionCode ?? ""]?.contains(key) == true {
            return false
        }
    ...
    // National public-holiday defaults for supported regions. State/province-only
    // holidays are intentionally excluded until OffTick has a region selector.
    private static let publicHolidays2026ByRegion: [String: Set<String>] = [
        "AU": [
